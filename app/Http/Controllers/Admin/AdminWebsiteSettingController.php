...
class AdminWebsiteSettingController extends Controller
{
    public function index()
    {
        ...
        // Decode the phone numbers from JSON format to an array
        $phones = [];
        if ($settings->phone) {
            $decodedPhones = json_decode($settings->phone, true);
            // Check if the decoded value is an array, otherwise, use an empty array
            if (is_array($decodedPhones)) {
                $phones = $decodedPhones;
            }
        }

        // Decode the fax number from JSON format to an array (if multiple fax numbers are stored)
        $faxes = [];
        if ($settings->fax) {
            $decodedFaxes = json_decode($settings->fax, true);
            if (is_array($decodedFaxes)) {
                $faxes = $decodedFaxes;
            }
        }
